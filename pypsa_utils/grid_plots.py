...
FIELDS = {
    "Line": ["bus0", "bus1", "s_nom", "length", "build_year"],
    "Link": ["bus0", "bus1", "p_nom", "length", "build_year"],
    "Bus": ["v_nom", "country", "carrier"],
}
ALIASES = {
    "Line": ["Bus 0", "Bus 1", "Capacity (MVA)", "Length (km)", "Commissioned"],
    "Link": ["Bus 0", "Bus 1", "P (nom.) (MW)", "Length (km)", "Commissioned"],
    "Bus": ["Nominal voltage (kV)", "Country", "Carrier"],
}
...
def _tooltip(fields, aliases):
    _tooltip = folium.GeoJson_tooltip(
        fields=fields, aliases=aliases, labels=True, localize=True, sticky=False
    )
    return _tooltip
...
def _get_linestring_from_buscoords(n):
    for branch in ["Line", "Link"]:
        n.df(branch).loc[:, "geometry"] = n.df(branch).apply(
            lambda x: LineString(
...
def plot_interactive_grid(n, tiles="CartoDB positron"):
    ...
    fg_lines = folium.FeatureGroup(name="Lines").add_to(m)
    fg_links = folium.FeatureGroup(name="Links").add_to(m)
    fg_buses = folium.FeatureGroup(name="Buses").add_to(m)
    ...
    folium.GeoJson(
        ...
        popup=lambda feature: _popup_content(
            feature=feature, fields=FIELDS["Line"], aliases=ALIASES["Line"]
        ),
    ).add_to(fg_lines)
    ...
    folium.GeoJson(
        ...
        popup=lambda feature: _popup_content(
            feature=feature, fields=FIELDS["Link"], aliases=ALIASES["Link"]
        ),
    ).add_to(fg_links)
    return m
